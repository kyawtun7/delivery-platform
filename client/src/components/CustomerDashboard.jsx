...
export default function CustomerDashboard({ user }) {
  ...
  const updateQuote = async (overrides = {}) => {
    ...
    try {
      ...
      const thb =
        res.data.priceTHB ?? Math.round(res.data.priceUSD * USD_TO_THB);
      setQuoteTHB(thb);
    } catch (err) {
      console.error(err);
      setQuoteTHB(null);
  ...
  // NEW: Clear history for a delivered order (delete from DB + from state)
  const handleClearHistory = async (id) => {
    const ok = window.confirm('Clear history for this order?');
    if (!ok) return;

    try {
      await api.delete(`/customer/deliveries/${id}`);
      // Remove from local state so UI updates instantly
      setDeliveries((prev) => prev.filter((d) => d.id !== id));
    } catch (err) {
      console.error(err);
      alert('Failed to clear history for this order');
    }
  };
            ...
            <div key={d.id} className="list-item">
              ...
              {d.status === 'delivered' && (
                <button
                  ...
                  type="button"
                  onClick={() => handleClearHistory(d.id)}
                >
                  Clear history
                </button>
              )}
